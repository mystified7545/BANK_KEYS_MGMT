...
gulp.task('clean:BANK_KEYS_MGMT', function (callback) {
  del.sync(paths.src.libs.dir);
  callback();
});

gulp.task('build', gulp.series(gulp.parallel('clean:BANK_KEYS_MGMT', 'copy:libs'), 'scss'));
gulp.task('default', gulp.series(gulp.parallel('clean:BANK_KEYS_MGMT', 'copy:libs', 'scss'), gulp.parallel('watch')));
